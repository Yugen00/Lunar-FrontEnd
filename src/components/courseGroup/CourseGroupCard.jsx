...
function CourseGroupCard({ index, data, handleDataChange, isSeeAll }) {
  ...
  const deleteHandle = async () => {
    try {
      const response = await customAxios.delete(
        `/courseGroup/Block/${data.GroupId}`
      );
    ...
    <>
      ...
      <div className="p-6 m-4 bg-white border rounded-lg shadow-md hover:shadow-lg transition-all duration-300">
        ...
        {!isSeeAll && <p 
          ...
          <b className="text-gray-600">Office Name:</b> {data?.OfficeName?.length > 15 ? (`${data.OfficeName.slice(0,15)} ...`):(data.OfficeName)}
        </p>}
        ...
        {!isSeeAll && <div className="mt-6 flex justify-start gap-4">
          {data.IsActive && (<Link to={`/courseGroup/update/${encodeURIComponent(tokenizedId)}`}>
