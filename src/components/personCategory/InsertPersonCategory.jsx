...
function InsertPersonCategory({ setOriginalData, setInsertModalOpen }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await customAxios.post('/personCategory/insert', formData);
      if (response.status == 200) {
        const newData = await response.data;
        setOriginalData((prev) => [...prev, newData]);
        setInsertModalOpen(false);
        showToast("Person Category Inserted Successfully", "success");
      }
    } catch (error) {
...
export default InsertPersonCategory;
