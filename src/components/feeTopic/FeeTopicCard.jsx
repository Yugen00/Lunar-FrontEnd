...
function FeeTopicCard({ index, data, handleDataChange, isSeeAll}) {
    ...
    <>
      ...
      <div className={`p-6 m-4 ${isSeeAll? ('sm:max-w-[50%]'):('')} bg-white border rounded-lg shadow-md hover:shadow-lg transition-all duration-300 `}>
        <h3 className="text-2xl font-semibold text-gray-800">
          {isSeeAll? (data.TopicName):(`${index + 1}.   ${data.TopicName}`)}
        </h3>
        ...
        {!isSeeAll && <div className="mt-6 flex justify-start gap-4">
          {data.IsActive && (<Link to={`/feeTopic/update/${encodeURIComponent(tokenizedId)}`}>
      ...
      {/* Confirmation Modal */}
      {isModalOpen && (
        <DeleteItem  handleModal={handleModal} deleteHandle={deleteHandle} name={data?.TopicName}/>
      )}
    </>
  );
}
